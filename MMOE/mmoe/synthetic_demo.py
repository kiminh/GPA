...
def data_preparation():
    ...
    for i in range(num_row):
        ...
        if num2 + comp2 + np.random.normal(scale=0.1, size=1)>-2:
            y1.append(1)
        else:
            y1.append(0)
...
def main():
    ...
    model.compile(
        loss={'y0': 'mean_squared_error', 'y1': 'binary_crossentropy'},
    ...
    res=np.array(res)
    m0=mean_squared_error(res[0],test_label[0])
    m1=mean_squared_error(res[1],test_label[1])
    ...
    plt.scatter(res[0],test_label[0])
    plt.show()
    plt.scatter(res[1],test_label[1])
    plt.show()
