...
if __name__ =="__main__":
    ...
    for file_name in os.listdir(grade_dir):
        ...
        course_info={}
        for course,credit,gpa,ctime,cat,stu_num in zip(gpa_df.course,gpa_df.credit,gpa_df.gpa,gpa_df.ctime,gpa_df.cat,gpa_df.stu_num):
            ctime=str(ctime)
            if ctime[4]=='s':#删去暑假学期
                continue
    ...
    train_stu_df=pd.DataFrame({'stu_id':train_stu_lst,'gender':train_gender_lst,'dep':train_dep_lst,'enroll_time':train_enroll_time_lst})
    train_stu_df.to_csv(train_stu_path,index=None)
    test_stu_df=pd.DataFrame({'stu_id':test_stu_lst,'gender':test_gender_lst,'dep':test_dep_lst,'enroll_time':test_enroll_time_lst})
    ...
    train_gpa_df=pd.DataFrame({'stu_id':train_gpa_stu_lst,'semester':train_sem_lst,'cname':train_gpa_m[:,0].flatten(),
    ...
    test_gpa_df=pd.DataFrame({'stu_id':test_gpa_stu_lst,'semester':test_sem_lst,'cname':test_gpa_m[:,0].flatten(),
